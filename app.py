...
if 'courses' not in st.session_state:
    st.session_state.courses = [
        ...
        ['校訂必修', '一般', '數學(校訂必修)', 0, 0, 2, 2, 2, 2, False], # 這是消失的高二數學！
...
with st.sidebar:
    st.header("👤 學生資訊")
    st_info = st.text_input("座號 / 姓名", placeholder="例如：01 黃萬瑄")
    if st.button("🧹 清空所有勾選"):
...
with st.expander("📥 點此展開：貼上成績文字自動勾選"):
    paste_txt = st.text_area("請貼上成績單文字：", height=100)
    if st.button("🚀 執行自動勾選"):
        if paste_txt:
            # 增加對 1121, 1122 等格式的支援
            sem_map = {"1121":0, "1122":1, "1131":2, "1132":3, "1141":4, "1142":5,
                       "112-1":0, "112-2":1, "113-1":2, "113-2":3, "114-1":4, "114-2":5}
            for line in paste_txt.split('\n'):
                for key, s_idx in sem_map.items():
                    if key in line:
                        for idx, row in enumerate(st.session_state.courses):
                            if row[2][:2] in line:
                                if row[3+s_idx] > 0: st.session_state[f"k_{idx}_{s_idx}"] = True
            st.success("✅ 解析完成！")
...
with c_l:
    ...
    bar("2. 部定必修 (125之85% 門檻 106.3)", d_s, 106.3)
with c_r:
...
with st.expander("🔍 點此展開：未取得學分科目清單"):
    if unpassed:
        for item in unpassed: st.write(item)
    else: st.success("🎉 目前無缺漏學分！")

if t_s >= 160 and d_s >= 106.3 and p_s >= 60 and s_s >= 30:
    st.balloons(); st.success(f"🎓 {st_info} 恭喜畢業！")
