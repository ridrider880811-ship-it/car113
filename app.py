...
with st.sidebar:
    ...
    if st.button("🧹 重置所有勾選"):
        ...
        st.rerun()
    is_mobile = st.checkbox("手機版檢視(單欄)", False)

with st.expander("📥 貼上成績文字自動偵測"):
    paste_txt = st.text_area("請在此貼上內容：", height=100)
    if st.button("🚀 開始偵測"):
        if paste_txt:
    ...
    with tab_obj:
        ...
        for i, row in enumerate(year_courses):
            ...
            with cols[i % num_cols]:
                ...
                # 使用 orig_idx + year_label + s_idx 組合出全球唯一 key，解決渲染衝突
...
d_cols = st.columns(4)
dash_data = [("🟢 總及格學分", total, 160), ("🔵 部定必修", dept, 106.3), ("🟠 專業與實習", prof, 60), ("🔴 純實習學分", prac, 30)]
...
for i, (label, current, target) in enumerate(dash_data):
    ...
    with d_cols[i]:
        st.markdown(f'<div class="metric-card"><div class="metric-label">{label}</div><div class="metric-value">{current} / {target}</div><div>{diff_html}</div></div>', unsafe_allow_html=True)
        st.progress(min(current/target, 1.0))

st.markdown("### 🔍 欠修/未及格科目")
cm1, cm2, cm3 = st.columns(3)
with cm1:
    with st.expander(f"{'🔴' if m1 else '🟢'} 一年級 {'缺修 ('+str(len(m1))+')' if m1 else '已達標'}", False):
        if m1: [st.markdown(f'<div class="missing-card">❌ {x}</div>', unsafe_allow_html=True) for x in m1]
        else: st.success("學分已全數取得")
with cm2:
    with st.expander(f"{'🔴' if m2 else '🟢'} 二年級 {'缺修 ('+str(len(m2))+')' if m2 else '已達標'}", False):
        if m2: [st.markdown(f'<div class="missing-card">❌ {x}</div>', unsafe_allow_html=True) for x in m2]
        else: st.success("學分已全數取得")
with cm3:
    with st.expander(f"{'⚠️' if m3 else '🟢'} 三年級 {'預計 ('+str(len(m3))+')' if m3 else '已達標'}", False):
