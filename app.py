...
# --- 1. 頁面配置與樣式 ---
st.set_page_config(page_title="汽車科畢業學分檢核系統", layout="wide")
...
st.caption("<div style='text-align:center;'>製作人：羅章成老師 | 113課綱</div>", unsafe_allow_html=True)
...
if 'courses' not in st.session_state:
    st.session_state.courses = [
        ...
        ['校訂選修', '一般', '原住民族語課程', 0, 0, 0, 2, 0, 0, False],
    ]

# --- 3. 側邊欄 ---
with st.sidebar:
    st.markdown("### 📋 學生資料")
    st_name = st.text_input("座號 / 姓名", value="", placeholder="例如：00號 000")
    if st.button("🧹 重置勾選"):
...
# --- 4. 偵測引擎 (終極抗黏貼：科目斬斷法) ---
with st.expander("📥 貼上成績", expanded=True):
    st.info("💡 **請將各學年成績分開貼入分頁**，系統能自動處理文字。")
    
    t_y1, t_y2, t_y3 = st.tabs(["📝 一年級", "📝 二年級", "📝 三年級"])
    with t_y1:
        txt_y1 = st.text_area("一年級", height=130, label_visibility="collapsed", placeholder="請貼上【一年級】成績總表...")
    with t_y2:
        txt_y2 = st.text_area("二年級", height=130, label_visibility="collapsed", placeholder="請貼上【二年級】成績總表...")
    with t_y3:
        txt_y3 = st.text_area("三年級", height=130, label_visibility="collapsed", placeholder="請貼上【三年級】成績總表...")
