...
# --- 2. 核心資料庫 (嚴格對照 113課綱新.pdf 逐行人工對位) ---
if 'courses' not in st.session_state:
    st.session_state.courses = [
        ...
        ['部定必修', '一般', '音樂', 0, 0, 1, 1, 0, 0, False],      # 絕對修正：二上1, 二下1
        ...
        ['校訂必修', '一般', '數學 (校訂必修)', 0, 0, 2, 2, 2, 2, False], # 消失的數學歸位
...
with st.expander("📥 點此展開：貼上成績文字自動勾選"):
    paste_txt = st.text_area("請直接貼上成績文字：", height=150)
    if st.button("🚀 執行自動勾選"):
        if paste_txt:
...
with c_l:
    st.write(f"**1. 總學分 (>=160)**")
    st.markdown(f"### {t_s} / 160")
    st.progress(min(t_s/160, 1.0))
    st.write(f"**2. 部定必修 (門檻 106.3)**")
    st.markdown(f"### {d_s} / 106.3")
    st.progress(min(d_s/106.3, 1.0))
with c_r:
    st.write(f"**3. 專業及實習科目 (>=60)**")
    st.markdown(f"### {p_s} / 60")
    st.progress(min(p_s/60, 1.0))
    st.write(f"**4. 純實習科目 (>=30)**")
    st.markdown(f"### {s_s} / 30")
    st.progress(min(s_s/30, 1.0))

with st.expander("🔍 點此展開：未取得學分科目清單"):
    if unpassed:
        for item in unpassed: st.markdown(f'<div class="missing-card">❌ {item}</div>', unsafe_allow_html=True)
    else: st.success("🎉 目前無缺漏學分！")

if t_s >= 160 and d_s >= 106.3 and p_s >= 60 and s_s >= 30:
    st.balloons(); st.success(f"🎓 {st_name} 恭喜畢業！")
