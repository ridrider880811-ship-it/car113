...
with st.sidebar:
    st.subheader("👤 學生資料")
    st_name = st.text_input("座號/姓名", value="") # 預設空白
    if st.button("🧹 重置所有勾選"):
...
with st.expander("📥 貼上成績文字自動偵測"):
    paste_txt = st.text_area("請在此貼上成績文字：", height=100)
    if st.button("🚀 開始偵測"):
        if paste_txt:
